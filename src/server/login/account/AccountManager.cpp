...
Account* AccountManager::validateAccountCredentials(LoginClient* client, const String& username, const String& password) {
	...
	if (account == nullptr) {
		...
		if (isAutoRegistrationEnabled() && client != nullptr) {
			...
			if(client != nullptr){
				...
				client->sendErrorMessage("Login Error", "Username not found.\nMake sure you have created both a Sentinels Republic Forum and Game Account.\nPlease sign in with your Game Account Credentials to play.\n\nVisit our forums or Discord for more information.");
			}
			return nullptr;
		}
	}
	...
	if ((passwordStored != passwordHashed) && (account->getSalt() != "")) {
