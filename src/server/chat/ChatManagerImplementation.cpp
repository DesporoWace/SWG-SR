...
void ChatManagerImplementation::initiateRooms() {
	...
	srRoom = createRoom("SR Galaxy", galaxyRoom);
	srRoom->setCanEnter(true);

	thePit = createRoom("The Pit (NSFW)", galaxyRoom);
	thePit->setCanEnter(true);
}
...
void ChatManagerImplementation::handleChatRoomMessage(CreatureObject* sender, const UnicodeString& message, unsigned int roomID, unsigned int counter) {
	...
	} else if (srRoom != NULL && srRoom->getRoomID() == roomID) {
			channel->broadcastMessageCheckIgnore(msg, name);
...
void ChatManagerImplementation::handleSRChat(CreatureObject* sender, const UnicodeString& message) {
	String name = sender->getFirstName();
	String fullName = "";
	...
	if (srRoom != NULL) {
		BaseMessage* msg = new ChatRoomMessage(fullName, server->getGalaxyName(), formattedMessage, srRoom->getRoomID());
		srRoom->broadcastMessageCheckIgnore(msg, name);
	}

}

void ChatManagerImplementation::handleTHEPit(CreatureObject* sender, const UnicodeString& message) {
	String name = sender->getFirstName();
	String fullName = "";
	...
	if (thePit != NULL) {
		BaseMessage* msg = new ChatRoomMessage(fullName, server->getGalaxyName(), formattedMessage, thePit->getRoomID());
		thePit->broadcastMessageCheckIgnore(msg, name);
	}
}
