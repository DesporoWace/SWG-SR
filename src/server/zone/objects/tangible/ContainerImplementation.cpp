...
void ContainerImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
	...
	if (checkContainerPermission(player, ContainerPermissions::MOVECONTAINER && getParent().get() != NULL && getParent().get()->checkContainerPermission(player, ContainerPermissions::MOVEOUT))){
		//Mindsoft added nameType, 0 = can be named, 1 = naming lock
		if (_this.getReferenceUnsafeStaticCast()->isRecycleToolObject() || _this.getReferenceUnsafeStaticCast()->isAntiDecayKitObject() || _this.getReferenceUnsafeStaticCast()->getNameType() == 1){
			return;
		}
