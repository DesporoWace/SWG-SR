			...
			if (zoneServer != NULL) {
				...
				if (target != NULL) {
					if (target->hasSkill("force_title_jedi_rank_02")) {
						...
						missionManager->updatePlayerBountyReward(targetId, 0);
						Core::getTaskManager()->scheduleTask([missionManager, targetId] {
							missionManager->removePlayerFromBountyList(targetId);
						}, "removeNonJediFromBountyList", 10000);
					}
				}
			}
			complete();
