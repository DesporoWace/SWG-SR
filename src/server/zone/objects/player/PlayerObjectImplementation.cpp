...
void PlayerObjectImplementation::notifySceneReady() {
	...
	//Join thePit
	ManagedReference<ChatRoom*> THEPit = chatManager->getthePit();
	if( THEPit == NULL )
	return;
	THEPit->sendTo(creature);
	THEPit->addPlayer(creature);
	chatManager->handleChatEnterRoomById(creature, THEPit->getRoomID(), -1, true);
...
void PlayerObjectImplementation::notifyOffline() {
	...
	*luaOnPlayerLoggedOut << playerCreature;
	...
	if (missionManager != nullptr && playerCreature->isPlayerCreature()) {
...
bool PlayerObjectImplementation::isIgnoring(const String& name) {
	String nameLower = name.toLowerCase();
	return !name.isEmpty() && ignoreList.contains(nameLower);
}
