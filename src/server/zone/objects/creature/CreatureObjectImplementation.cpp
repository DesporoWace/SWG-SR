...
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* object, bool bypassDeadCheck) {
	...
	if (object->isAiAgent()) {
		...
		if (object->isPet()) {
			...
			if (pcd != nullptr && pcd->getPetType() == PetManager::FAMILIARPET) {
				return false;
			}
			...
			if (pcd != nullptr && pcd->getPetType() == PetManager::FACTIONPET) {
				if ((pvpStatusBitmask & CreatureFlag::OVERT) && (owner->getPvpStatusBitmask() & CreatureFlag::OVERT) && owner->getFaction() != getFaction())
