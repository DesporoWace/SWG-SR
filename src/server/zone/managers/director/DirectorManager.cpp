...
void DirectorManager::initializeLuaEngine(Lua* luaEngine) {
	...
	// SR
	lua_register(luaEngine->getLuaState(), "updateLeaderboards", updateLeaderboards);
	...
	Luna<LuaSeaRemovalFuel>::Register(luaEngine->getLuaState());
}
...
int DirectorManager::broadcastGalaxy(lua_State* L) {
	...
	if (numberOfArguments == 2) {
		...
		chatManager->broadcastGalaxy(faction, message);
	} else {
		chatManager->broadcastGalaxy(NULL, message);
	}
...
int DirectorManager::updateLeaderboards(lua_State* L) {
	int numberOfArguments = lua_gettop(L);
	if (numberOfArguments != 4) {
		String err = "incorrect number of arguments passed to DirectorManager::updateLeaderboards";
		printTraceError(L, err);
		ERROR_CODE = INCORRECT_ARGUMENTS;
		return 0;
	}
	...
	SceneObject* player = (SceneObject*) lua_touserdata(L, -4);
	// Event Type
	String event_type = lua_tostring(L, -2);
	// Event Extra
	String event_extra = lua_tostring(L, -1);

	if (event_type == "") {
		DirectorManager::instance()->error("Attempted to update leaderboards with an empty event type field.");
		return 0;
	}


	if (player == NULL || !player->isPlayerCreature()) {
		DirectorManager::instance()->error("Attempted to update leaderboards of a non-player Scene Object.");
		return 0;
	}
	...
	if (ghost == NULL) {
		DirectorManager::instance()->error("updateLeaderboards failed to create ghost as player object.");
		return 0;
	}

	Reference<PlayerObject*> target = ghost;

	// Target
	if (event_type != "elite_kill" && event_type != "wb_kill" && event_type != "mob_kill") {
		SceneObject* targetobj = (SceneObject*) lua_touserdata(L, -3);

		if (targetobj == NULL || !targetobj->isPlayerCreature()) {
			DirectorManager::instance()->error("Attempted to update leaderboards of the target non-player Scene Object.");
			return 0;
		}

		Reference<PlayerObject*> target = player->getSlottedObject("targetobj").castTo<PlayerObject*>();
	...
	if (event_extra == "") {
		DirectorManager::instance()->error("Attempted to update leaderboards with an empty event extra field.");
		return 0;
	}

	ManagedReference<PlayerManager*> playerManager = ServerCore::getZoneServer()->getPlayerManager();

	playerManager->updateLeaderboards(ghost, target, event_type, event_extra);
