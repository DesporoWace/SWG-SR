...
bool GroupManager::canJoinGroup(GroupObject* group, CreatureObject* inviter, CreatureObject* target) {
	...
	if (target->isPlayerObject()) {
		...
		if (ghostInviter == NULL || ghostTarget == NULL)
			return false;

		if (!ghostInviter->isStaff()) {
			if (ghostTarget->isStaff() && group->getGroupSize() < adminGroupMax)
				return true;
			else if (group->getGroupSize() >= playerGroupMax)
				return false;
		} else
			if (group->getGroupSize() >= adminGroupMax)
				return false;
	} else {
		if (ghostInviter == NULL || target == NULL)
			return false;

		if (!ghostInviter->isStaff()) {
			if (group->getGroupSize() >= playerGroupMax)
				return false;
...
void GroupManager::inviteToGroup(CreatureObject* leader, CreatureObject* target) {
	...
	if (leader->isGrouped()) {
		...
		if (!canJoinGroup(group, leader, target)) {
...
void GroupManager::joinGroup(CreatureObject* player) {
	...
	if (!canJoinGroup(group, inviter, player)) {
		clocker.release();

		player->updateGroupInviterID(0);

		player->sendSystemMessage("@group:join_full_group"); // The group you are attempting to join is full.
		return;
	}
...
void GroupManager::makeLeader(GroupObject* group, CreatureObject* player, CreatureObject* newLeader) {
	...
	try {
		...
		if (group->getGroupSize() > playerGroupMax) {
			Reference<PlayerObject*> ghost = newLeader->getSlottedObject("ghost").castTo<PlayerObject*>();
			if (ghost != NULL && !ghost->isStaff()) {
				player->sendSystemMessage("@group:leader_lacks_permission"); // Group member lacks the permissions to be leader of a group this size.
				group->unlock();
				player->wlock();
				return;
			}
