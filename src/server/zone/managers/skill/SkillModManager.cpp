...
void SkillModManager::verifyWearableSkillMods(CreatureObject* creature) {
	...
	for(int i = 0; i < creature->getSlottedObjectsSize(); ++i) {
		...
		if(object->isWearableObject() && (object->getMaxCondition() - object->getConditionDamage()) > 0){
		...
		} else if (object->isWearableContainerObject() && (object->getMaxCondition() - object->getConditionDamage()) > 0) {
